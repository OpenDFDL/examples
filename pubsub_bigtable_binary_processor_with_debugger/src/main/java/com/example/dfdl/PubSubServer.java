...
public class PubSubServer {
    ...
    return message -> {
      ...
      try {
        ...
        messagingGateway.sendToPubsub(pubsubDataJsonTopic, messageConverted);
               ...
               ValidatorInitializationException usageException) {
        usageException.printStackTrace();
      }
  ...
  public MessageHandler messageSender(PubSubTemplate pubsubTemplate) {
    ...
    adapter.setSuccessCallback((message, result) -> {
      LOGGER.info(
        "Message was sent via the outbound channel adapter to a topic "
      );
    });
    adapter.setFailureCallback((message, throwable) -> {
